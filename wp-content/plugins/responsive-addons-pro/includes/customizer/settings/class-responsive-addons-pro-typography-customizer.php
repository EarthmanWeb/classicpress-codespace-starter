...
if ( ! class_exists( 'Responsive_Addons_Pro_Typography_Customizer' ) ) :
	/**
	 * Customizer Typography Loader
	 *
	 * @since 1.0.0
	 */
	class Responsive_Addons_Pro_Typography_Customizer {
		/**
		 * Setup class.
		 */
		...
		public function elements() {
			...
			$responsive_typography_settings = array(
				'page_title'       => array(
				...
				'blog_post_title'  => array(
					'label'    => esc_html__( 'Post Title', 'responsive' ),
					'target'   => '.single-post #primary .post .post-entry .entry-title',
					'section'  => 'responsive_post_typography',
					'priority' => 10,
					'defaults' => array(
						'font-size'      => '34px',
						'color'          => '#333333',
				...
				'blog_entry_title' => array(
					'label'    => esc_html__( 'Blog Title', 'responsive' ),
			...
			if ( ! $this->is_version_greater( 'responsive-pro' ) ) {
				$responsive_typography_settings['page_title'] = array(
					'label'    => esc_html__( 'Page Title', 'responsive' ),
					'target'   => '.page .post-title, #main-blog h1',
					'section'  => 'responsive_page_typography',
					'priority' => 10,
		...
		public function customizer_options( $wp_customize ) {
			...
			if ( 'Responsive' === $theme->name || 'Responsive' === $theme->parent_theme ) {
				...
				$wp_customize->add_section(
					'responsive_post_typography',
					array(
						'title'    => esc_html__( 'Typography', 'responsive' ),
						'panel'    => 'responsive_single_blog_layout',
						'priority' => 160,
					)
				);
				$wp_customize->add_section(
					'responsive_blog_typography',
					array(
						'title'    => esc_html__( 'Typography', 'responsive' ),
						'panel'    => 'responsive_blog_layout',
						'priority' => 230,
					)
				);
				...
				foreach ( $elements as $element => $array ) {
					...
					if ( $label ) {
						...
						responsive_pro_separator_control( $wp_customize, $element . '_seperator', $label, $section, $seperator_priority );
		...
		public function customize_preview_init() {
			$path = plugin_dir_url( __FILE__ ) . '../assets/js/responsive-pro-typography-customize-preview.js';
			wp_enqueue_script( 'responsive-typography-customize-preview', $path, array( 'customize-preview' ), '1.1', true );
			wp_localize_script(
		...
		public function loop( $return = 'css' ) {
			...
			if ( 'css' === $return && ! empty( $css ) ) {
				$css = '/* Responsive Pro Typography CSS */' . $css;
				return $css;
			}
...
return new Responsive_Addons_Pro_Typography_Customizer();
