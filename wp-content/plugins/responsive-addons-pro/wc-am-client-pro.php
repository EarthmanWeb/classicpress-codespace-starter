	...
	class WC_AM_Client_2_7_PRO {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
			...
			if ( is_admin() ) {
				...
				/**
				 * Conditionally add the action if the responsive theme is not activated with version greater than 4.8.8
				 *
				 * @since 2.5.1
				 */
				if ( ! $this->responsive_activated ) {
		...
		public function inactive_notice() { ?>
			...
			<?php if ( apply_filters( 'wc_am_client_inactive_notice_override', true ) ) { ?>
				...
				<?php if ( isset( $_GET[ 'page' ] ) && ($this->wc_am_activation_tab_key == $_GET[ 'page' ] || 'responsive' == $_GET['page']) ) {
					return;
				} ?>
					<?php
					if ( $this->responsive_activated ) {
						?>
						<div class="notice notice-error">
                    		<p><?php printf( __( 'The <strong>%s</strong> API Key has not been activated, so the %s is inactive! %sClick here%s to activate <strong>%s</strong>.', $this->text_domain ), esc_attr( $this->software_title ), esc_attr( $this->plugin_or_theme ), '<a href="' . esc_url( admin_url( 'themes.php?page=responsive#settings' ) ) . '">', '</a>', esc_attr( $this->software_title ) ); ?></p>
						</div>
						<?php
					}
					?>
		...
		public function activate( $args, $product_id = 0 ) {
			...
			if ( 0 !== $product_id && isset( $_POST['productId'] ) && ! empty( $_POST['productId'] ) ) {
